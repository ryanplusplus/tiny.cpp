...
TEST_GROUP(Queue)
{
  ...
  {
    queue = make_unique<Queue>(buffer, sizeof(buffer));
  }

  void given_that_small_element_has_been_enqueued(uint8_t element)
  {
    CHECK_TRUE(queue->enqueue(reinterpret_cast<const void*>(&element), sizeof(element)));
  }
  ...
  void small_element_shoulde_be_dequeued(uint8_t element)
  {
    ...
    queue->dequeue(&dequeued_element_storage, &dequeued_element_size);
  ...
  {
    return queue->enqueue(reinterpret_cast<void*>(element), large_element_size);
  }
  ...
  void peeking_should_yield(uint8_t * element, size_t element_size, uint16_t element_index)
  {
    ...
    queue->peek(&peekedElementStorage, &dequeued_element_size, element_index);
  ...
  void enqueue_one_byte_elements_such_that_each_element_is_incremented(uint8_t startingNumber, uint8_t element_count)
  {
    ...
    for(uint8_t count = 0; count < element_count; count++) {
      queue->enqueue(&element, sizeof(element));
      element++;
    }
  }
  ...
  {
    CHECK_EQUAL(expected, queue->count());
  }
};

TEST(Queue, should_be_able_to_enqueue_and_dequeue_a_small_element)
{
...
TEST(Queue, should_be_able_to_enqueue_and_discard_a_small_element)
{
  given_that_the_queue_has_been_initialized();
  given_that_small_element_has_been_enqueued(some_small_element);
...
TEST(Queue, should_be_able_to_enqueue_and_dequeue_a_large_element)
{
  given_that_the_queue_has_been_initialized();
  given_that_large_element_has_been_enqueued(some_large_element_1);
...
TEST(Queue, should_be_able_to_enqueue_three_elements_and_then_dequeue_them)
{
  given_that_the_queue_has_been_initialized();
  given_that_small_element_has_been_enqueued(some_small_element);
  given_that_large_element_has_been_enqueued(some_large_element_1);
...
TEST(Queue, should_be_able_to_discard_some_elements_and_then_dequeue_others)
{
  given_that_the_queue_has_been_initialized();
  given_that_small_element_has_been_enqueued(some_small_element);
  given_that_large_element_has_been_enqueued(some_large_element_1);
...
TEST(Queue, should_be_able_to_enqueue_more_elements_after_dequeuing_and_discarding)
{
  given_that_the_queue_has_been_initialized();
  given_that_small_element_has_been_enqueued(some_small_element);
  given_that_large_element_has_been_enqueued(some_large_element_1);
...
TEST(Queue, should_not_enqueue_elements_that_will_not_fit_in_the_queue)
{
  given_that_the_queue_has_been_initialized();
...
TEST(Queue, should_be_able_to_peek_at_an_element_multiple_times)
{
  given_that_the_queue_has_been_initialized();
  given_that_large_element_has_been_enqueued(some_large_element_1);
...
TEST(Queue, should_be_able_to_peek_at_any_element_that_is_queued)
{
  given_that_the_queue_has_been_initialized();
...
TEST(Queue, should_be_able_to_peek_at_an_element_and_then_dequeue_it)
{
  given_that_the_queue_has_been_initialized();
  given_that_large_element_has_been_enqueued(some_large_element_1);
  peeking_from_the_front_of_the_queue_should_yield(some_large_element_1);
...
TEST(Queue, should_be_able_to_peek_just_part_of_an_element)
{
  given_that_the_queue_has_been_initialized();
  given_that_large_element_has_been_enqueued(some_large_element_1);
...
TEST(Queue, should_be_able_to_peek_size_of_any_element_that_is_queued)
{
  given_that_the_queue_has_been_initialized();
  given_that_large_element_has_been_enqueued(some_large_element_1);
...
TEST(Queue, should_know_how_many_elements_are_queued)
{
  given_that_the_queue_has_been_initialized();
...
TEST(Queue, should_know_how_many_elements_are_queued_after_some_have_been_dequeued)
{
  given_that_the_queue_has_been_initialized();
  given_that_n_elements_have_been_enqueued(4);
...
TEST(Queue, should_know_how_many_elements_are_queued_after_some_have_been_discarded)
{
  given_that_the_queue_has_been_initialized();
  given_that_n_elements_have_been_enqueued(4);
